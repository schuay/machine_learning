...
plot_train_time <- function(file_names, classifiers) {
    figbasename = strsplit(as.character(runif(1, 0, 1)), "\\.")[[1]][2]
    figfile <- paste(figbasename, ".eps", sep = "")
    ...
    p <- ggplot(data, aes(x=dataset, y=mean_train_time, fill=classifier)) +
            geom_bar(position=position_dodge(), stat="identity")
...
plot_test_time <- function(file_names, classifiers) {
    figbasename = strsplit(as.character(runif(1, 0, 1)), "\\.")[[1]][2]
    figfile <- paste(figbasename, ".eps", sep = "")
    ...
    p <- ggplot(data, aes(x=dataset, y=mean_test_time, fill=classifier)) +
            geom_bar(position=position_dodge(), stat="identity")
