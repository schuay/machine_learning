...
We used a selection of datasets to test our classifiers, consisting of:

\begin{enumerate}
\item \annealing: steel annealing,

\item \digits: handwritten digit recognition,
\item \iris: iris taxonomy,

\item \hastie: randomly generated binary classification as in Hastie et al. 2009,
\item \randomtwo: randomly generated data with 2 classes,
\item \randomten: randomly generated data with 10 classes,

\item \breastcancer: classification according to symptoms,
\item \vehicle: no information provided,
\item \heart: heart disease diagnosis,
\item \connectfour: outcome prediction for a game of connect-4,
\item \vowel: speech recognition,
\end{enumerate}

Dataset 1 is taken from \url{http://archive.ics.uci.edu/ml/}, sets 2-3 are
fetched and imported using \verb|scikit-learn| functions, sets 4-6 are randomly
generated using \verb|scikit-learn| functions, and finally sets 7-11 are taken from
the \verb|libSVM| dataset collection at \url{http://www.csie.ntu.edu.tw/~cjlin/libsvmtools/datasets/}.
...
For the \weightedmaj classifiers we also used the mean accuracy reported for a component
as the weight in the majority function. The use of cross-validation to obtain this weight
makes a separate validation data set superfluous\footnote{see
...
Figures~\ref{fig:randomtwo} through \ref{fig:connectfour} show the results of our
ensemble classifiers for each of our eleven datasets. In a) through f) the mean accuracies
and standard deviations of each ensemble classifier is compared to that of each component. In order
to make it easier to discern differences between classifiers, note that all axes
are scaled from $0.5$ to $1.0$. Any classifiers producing accuracy less than $0.5$ do not show
up on the graph.
...
Results were very similar for many of our datasets, and thus we only examine the most
interesting graphs here in order to save space. In particular, figures for \annealing and
\breastcancer were omitted since all classifiers were uniformly very effective.
Likewise, \vowel and \randomten were omitted since all classifiers performed poorly
and results were mostly off the chart (with a notable exception of the nearest neighbor
classifier, which produced the best results in both of these difficult multi-class problems).
\vehicle was omitted due to its similarity to \randomtwo.
...
\caption{\randomtwo\label{fig:randomtwo}}
\end{figure}
...
and testing time diagrams, being only slightly slower than a single tree.
The \extraforest ensemble does not share this property, as it is significantly slower
during training.

The chosen majority function on the other hand does not seem to make a discernable
or consistent difference in most cases. The lower variance of the weighted ensemble
in the \heart dataset can be explained by the inherent randomization of the
classifier.

It is interesting to note how the behavior of classifier families (Bayes, nearest neighbors, etc.)
differs according to the dataset. For instance, naive Bayes classifiers perform very competitively on
most datasets (most impressively on \hastie), but produce poor results on \connectfour. In contrast, support
vector classifiers have almost the opposite behavior, and completely fail to perform on the \hastie
dataset.

Finally, support vector and naive Bayes classifiers always produce very consistent results over all
component classifiers of the ensemble. The variation between decision trees and nearest neighbor
classifiers is larger, leaving a higher potential for ensemble methods. Ensembles
show the most improvements over their components when using extremely random trees,
which is not surprising as these classifiers were explicitly intended for use in ensemble methods.
...
@
\caption{Ensemble classifier accuracy\label{fig:allensacc}}
\end{figure}

% --------------------------------------------------------------------------------------------------

Figures~\ref{fig:mixacc} through \ref{fig:treesmallacc} compare each ensemble classifier
with its respective components for all four datasets. Again, only the most interesting
figures are shown, with very similar results such as naive Bayes, nearest neighbor,
and support vector machines being left out as ensemble techniques did not result
in any improvements.
...
plot_accuracy(c("../results/random10.csv", "../results/vowel.scale.csv", "../results/heart_scale.csv",
		...
		c("mix_default_simple_ensemble", "mix_default_weighted_ensemble", "nb0", "knn0", "svm0", "tree0", "extratree0"))
@
\caption{Mixed ensemble accuracy\label{fig:mixacc}}
\end{figure}
...
plot_accuracy(c("../results/random10-supreme.csv", "../results/vowel.scale-supreme.csv",
		"../results/heart_scale-supreme.csv", "../results/digits-supreme.csv"),
		c("extraforest2_simple_ensemble", "extraforest2_weighted_ensemble", "t0", "t1", "t2", "t3", "t4", "t5", "t6", "t7"))
@
\caption{Forest of 30 trees accuracy}
\end{figure}
...
this is even more pronounced in the \digits dataset. Note also how the variance is greatly reduced
in these cases over the base components.
...
However, the best accuracy is still obtained by simply using a single base classifier.
For example, for \digits, a nearest neighbor classifier is obviously the best choice, while
naive Bayes produces the best accuracy on the \heart dataset.
...
plot_accuracy_by_train_time(c("../results/random10.csv",
		...
		c("mix_default_simple_ensemble", "nb7_simple_ensemble", "knn7_simple_ensemble",
			...
			"nb0", "knn0", "svm0", "tree0", "extratree0"))
@
\caption{\randomten: Train time vs. accuracy}
\end{figure}
...
plot_accuracy_by_test_time(c("../results/random10.csv",
		...
		c("mix_default_simple_ensemble", "nb7_simple_ensemble", "knn7_simple_ensemble",
			...
			"nb0", "knn0", "svm0", "tree0", "extratree0"))
@
\caption{\randomten: Test time vs. accuracy\label{fig:randomtentest}}
\end{figure}

\clearpage

These runtime figures show that forests compare favourably with the baseline tree
in regards to accuracy. However, when only a subset of the training set is used for each
tree component (as is the case in \extraforest, \extraforesttwo and \treesmall), the
runtime of the ensemble classifier dramatically increases, due to a slow
algorithm for the random subset selection. With increasing dataset size, training time
again dominates.
...
performance over a single classifier are forests. In many cases, the resulting accuracies
are comparable to the best single classifiers. We achieved these improvements by both
...
subsets, coupled with the lost accuracy of base components caused by the smaller training set
may offset any performance gain.

However, forests seem to be fairly stable over different
problem sets; i.e. unlike single classifiers, which sometimes perform poorly on one dataset
while excelling on another, forests seem to consistently produce accuracies in the
upper middle ranges and may be very well suited when reliable performance is more
important than optimal accuracy.
