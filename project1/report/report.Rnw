...
\begin{itemize}
\item combining several different classification algorithms in an ensemble classifier,
\item implementing two strategies - simple majority and weighted majority voting - for use in this ensemble classifier,
\item create a suite of different datasets and classifier ensembles,
\item evaluate classifier and ensemble classifier performance and efficiency on this suite,
\item and finally provide a user interface based on a configuration file to specify the classifier ensembles and test suite.
\end{itemize}
...
%TODO
We selected the following classifiers and configuration options:
\begin{itemize}
	\item KNN
	\item Naive Bayes
	\item SVM
...
%TODO
The methods to combine our classifiers were given in the specification. We implemented one
ensemble classifier which performs a simple majority vote on the predictions of the
classifiers (called \simplemaj) and one which uses a weighted majority vote (called
\weightedmaj).

However, we were free to choose which combinations of classifiers we should evaluate.
...
The ensemble is specified via an INI-style configuration file. Each section header
contains a classifier name. The Python class which provides the underlying algorithm is
specified with the \texttt{kind} option. All other options in a section specify options
specific to this class.

Listing~\ref{lst:config} shows the default config file which resides in
\lstinline|src/conf|.

\lstinputlisting[label=lst:config,style=BashOutputStyle,caption=src/conf/default.conf]{../src/conf/default.conf}
