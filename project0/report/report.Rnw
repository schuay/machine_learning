...
SGD regressor above and using it on large datasets is now always feasible.
...
All regression algorithms performed poorly when the dataset was read in chonological order.
...
very bad results and therefore \noscale regressor was not used for this dataset.
