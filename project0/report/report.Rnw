...
This technique attempts to minimize a loss function. %TODO
This regressor is particularly useful for large datasets and is faster than many of the alternatives. Additionally,
we were already familiar with Stochastic Gradient Descent from the lecture.
...
The \powerconsumption and \solarflares benchmarks were performed on a machine with 64 GB of RAM and
2 Intel Xeon processors. % TODO add exact CPU type and kernel version
